...
const StockArticles = ({ selectedStock, stockName, featured }) => {
  const auth = useAuth();
    ...
    const fetchNews = async () => {
      try {
        const response = await axios.get(
          `${featured ? "/api/news/featured" : "api/news"}`,
          {
  ...
  return (
    <div className='w-full mt-16'>
      <h1 className='text-primary text-3xl font-semibold'>Featured Articles</h1>
